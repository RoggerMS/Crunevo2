...
def mentorship_dashboard():
    ...
    my_requests = Mentorship.query.filter_by(
        student_id=current_user.id, status=MentorshipStatus.PENDING
    ).all()

    # Get active mentorships as student
    my_mentorships = Mentorship.query.filter_by(
        student_id=current_user.id, status=MentorshipStatus.ACTIVE
    ).all()
    ...
    if current_user.forum_level >= 5:
        mentor_requests = Mentorship.query.filter_by(
            mentor_id=current_user.id, status=MentorshipStatus.PENDING
        ).all()
...
def request_mentorship(mentor_id):
    ...
    existing_mentorship = (
        Mentorship.query.filter_by(student_id=current_user.id, mentor_id=mentor_id)
        .filter(
...
def study_groups():
    ...
    return render_template(
        "social/study_groups.html", study_groups=study_groups, my_groups=my_groups
    )
...
def create_study_group():
    ...
    member = StudyGroupMember(group_id=group.id, user_id=current_user.id)
...
def competitions():
    ...
    upcoming_competitions = (
        Competition.query.filter(Competition.start_date > now)
...
def claim_challenge_reward(challenge_id):
    ...
    try:
        result = CrolarsIntegrationService.claim_challenge_reward(
        ...
        return (
            jsonify({"success": False, "message": "Error al reclamar la recompensa"}),
            500,
        )
...
def leaderboards():
    ...
    level_leaderboard = (
        User.query.filter(User.level > 0)
        .order_by(User.level.desc(), User.forum_experience.desc())
        .limit(10)
        .all()
    )
    ...
    helpful_answers_leaderboard = (
        User.query.filter(User.helpful_answers_count > 0)
        .order_by(User.helpful_answers_count.desc())
        .limit(10)
        .all()
    )
    ...
    user_level_rank = User.query.filter(User.level > current_user.level).count() + 1

    user_reputation_rank = (
        User.query.filter(User.reputation_score > current_user.reputation_score).count()
        + 1
    )
    ...
    user_crolars_rank = (
        User.query.filter(User.credits > current_user.credits).count() + 1
    )
...
def social_feed():
    ...
    recent_answers = (
        ...
        .order_by(ForumAnswer.created_at.desc())
        .limit(10)
        .all()
    )
    ...
    accepted_answers = (
        ForumAnswer.query.filter(
            ForumAnswer.is_accepted,
            ForumAnswer.updated_at >= datetime.utcnow() - timedelta(days=7),
        )
        .order_by(ForumAnswer.updated_at.desc())
        .limit(5)
        .all()
    )
...
def crolars_stats():
    ...
    return render_template(
        "social/crolars_stats.html", stats=stats, leaderboard=leaderboard
    )
...
def buy_premium_feature():
    ...
    if feature_type == "custom_title":
        ...
        if success:
            ...
            db.session.commit()
            flash(
                f"Título personalizado '{custom_title}' activado exitosamente",
                "success",
            )
        else:
        ...
        success = CrolarsIntegrationService.use_premium_feature(
            current_user.id, feature_type, question_id=question_id, answer_id=answer_id
        )
        ...
        if success:
            ...
            flash(
                f"Función de {feature_names.get(feature_type, feature_type)} activada exitosamente",
                "success",
            )
        else:
