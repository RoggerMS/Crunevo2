...
def dashboard():
    ...
    # Club statistics
    most_active_clubs = db.session.query(Club).order_by(Club.member_count.desc()).limit(5).all()

    # Financial stats
    total_crolars_circulation = db.session.query(db.func.sum(User.credits)).scalar() or 0
...
def manage_reports():
    ...
    return render_template('admin/manage_reports.html', reports=reports, current_status=status)
...
def export_users():
    ...
    # Header
    writer.writerow(['ID', 'Username', 'Email', 'Credits', 'Points', 'Verified', 'Created'])
