...
def list_saved():
    ...
    saved_notes = db.session.query(SavedContent, Note).join(
        Note, SavedContent.content_id == Note.id
    ).filter(
        SavedContent.user_id == current_user.id,
        SavedContent.content_type == 'note'
    ).order_by(SavedContent.saved_at.desc()).all()
    
    saved_posts = db.session.query(SavedContent, Post).join(
        Post, SavedContent.content_id == Post.id
    ).filter(
        SavedContent.user_id == current_user.id,
        SavedContent.content_type == 'post'
    ).order_by(SavedContent.saved_at.desc()).all()
    
    return render_template('saved/list.html', 
