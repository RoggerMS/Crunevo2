...
clubot_bp = Blueprint('clubot', __name__)

# Clubot knowledge base
CLUBOT_RESPONSES = {
    "que_es_crunevo": {
        "keywords": ["qué es crunevo", "que es crunevo", "crunevo", "plataforma", "explicar"],
    ...
    "como_ganar_crolars": {
        "keywords": ["cómo ganar crolars", "como ganar crolars", "crolars", "ganar dinero", "moneda", "créditos"],
    ...
    "donde_clubes": {
        "keywords": ["dónde están los clubes", "donde estan los clubes", "clubes", "unirme club", "encontrar club"],
    ...
    "ganar_dinero": {
        "keywords": ["puedo ganar dinero", "dinero real", "monetizar", "pagar", "cobrar"],
    ...
    "como_subir_apuntes": {
        "keywords": ["cómo subir apuntes", "como subir apuntes", "subir notas", "cargar archivos", "apuntes"],
    ...
    "foro_preguntas": {
        "keywords": ["cómo hacer preguntas", "como hacer preguntas", "foro", "preguntar", "ayuda académica"],
...
def find_best_response(user_message):
    ...
    for response_key, data in CLUBOT_RESPONSES.items():
        score = 0
...
@clubot_bp.route('/clubot')
def clubot_chat():
    """Clubot chat interface"""
    return render_template('clubot/chat.html')


@clubot_bp.route('/api/clubot/message', methods=['POST'])
def clubot_message():
    """Process message and return Clubot response"""
