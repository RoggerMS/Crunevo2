...
from crunevo.models.hall_1000 import (
    CrolarsHallMember, PremiumContent, HallRaffle, RaffleParticipant
)
...
def index():
    ...
    if not member:
        ...
        total_spent = db.session.query(db.func.sum(Purchase.total_price)).filter(
            Purchase.user_id == current_user.id,
            Purchase.status == 'completed'
        ).scalar() or 0
        
        if total_spent < 1000:
            return render_template('hall/locked.html', 
                                 total_spent=total_spent,
                                 needed=1000 - total_spent)
        else:
            # Unlock access
            member = unlock_hall_access(current_user.id, total_spent)
    
    # Get premium content
    premium_content = PremiumContent.query.filter(
        PremiumContent.is_active == True,
        PremiumContent.required_level <= get_access_level(member.total_spent)
    ).order_by(PremiumContent.created_at.desc()).all()
    
    # Get active raffles
    active_raffles = HallRaffle.query.filter(
        HallRaffle.is_active == True,
    ...
    return render_template('hall/index.html',
                         member=member,
...
def premium_content():
    ...
    query = query.filter(
        PremiumContent.required_level <= user_level
    )
    
    content = query.order_by(PremiumContent.created_at.desc()).all()
    
    return render_template('hall/premium_content.html',
                         member=member,
                         content=content,
...
def raffles():
    ...
    completed_raffles = HallRaffle.query.filter(
        HallRaffle.is_completed == True
    ).order_by(HallRaffle.end_date.desc()).limit(10).all()
    
    return render_template('hall/raffles.html',
                         member=member,
                         active_raffles=active_raffles,
...
def join_raffle(raffle_id):
    ...
    participation = RaffleParticipant(
        raffle_id=raffle_id,
        user_id=current_user.id,
        entries_used=raffle.entry_cost
    )
...
def my_progress():
    ...
    purchases = Purchase.query.filter(
        Purchase.user_id == current_user.id,
        Purchase.status == 'completed'
    ).order_by(Purchase.created_at.desc()).limit(20).all()
    
    # Calculate next level requirements
    next_level_spent = get_next_level_requirement(member.access_level)
    
    return render_template('hall/my_progress.html',
                         member=member,
                         purchases=purchases,
