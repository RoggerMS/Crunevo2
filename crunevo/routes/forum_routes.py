...
def list_questions():
    ...
    categories = ['Matemáticas', 'Ciencias', 'Lenguas', 'Historia', 'Tecnología', 'Arte', 'Otros']
    
    return render_template('forum/list.html', questions=questions, categories=categories, current_category=category)
...
def view_question(question_id):
    ...
    answers = ForumAnswer.query.filter_by(question_id=question_id).order_by(
        ForumAnswer.is_accepted.desc(), ForumAnswer.votes.desc(), ForumAnswer.created_at.asc()
    ).all()
...
def ask_question():
    if request.method == 'POST':
        ...
        add_credit(current_user, 3, CreditReasons.ACTIVIDAD_SOCIAL, related_id=question.id)
