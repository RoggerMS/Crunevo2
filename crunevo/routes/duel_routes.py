...
def list_duels():
    recent_duels = (
        AcademicDuel.query.filter(AcademicDuel.status.in_(["answered", "validated"]))
        .order_by(AcademicDuel.created_at.desc())
        .limit(20)
        .all()
    )
