...
def get_apuntes():
    ...
    query = Note.query.join(User).filter(
        User.career == current_user.career
    )
...
def get_cursos():
    ...
    courses = Course.query.filter(
        Course.category == current_user.career
    ).order_by(Course.views.desc()).limit(20).all()
...
def get_clubes():
    ...
    clubs = Club.query.filter(
        Club.career == current_user.career
    ).order_by(Club.member_count.desc()).all()
...
def get_eventos():
    ...
    events = Event.query.filter(
        ...
        or_(
            Event.category == current_user.career,
...
def get_chat_messages():
    ...
    messages = Message.query.join(User).filter(
        User.career == current_user.career,
        Message.is_global == True  # Using global chat filtered by career
    ...
    active_users = User.query.filter(
        ...
        User.id.in_(
            db.session.query(Message.sender_id).filter(
                Message.timestamp >= datetime.utcnow() - timedelta(minutes=10)
            ).distinct()
        )
    ).count()
