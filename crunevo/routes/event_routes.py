...
def list_events():
    ...
    upcoming_events = Event.query.filter(Event.event_date > func.now()).order_by(Event.event_date.asc()).all()
    past_events = Event.query.filter(Event.event_date <= func.now()).order_by(Event.event_date.desc()).limit(5).all()
    
    user_participations = []
    if current_user.is_authenticated:
        user_participations = [p.event_id for p in EventParticipation.query.filter_by(user_id=current_user.id).all()]
...
def view_event(event_id):
    ...
    if current_user.is_authenticated:
        is_participating = EventParticipation.query.filter_by(
            user_id=current_user.id, 
            event_id=event_id
        ).first() is not None
    
    recent_participants = EventParticipation.query.filter_by(event_id=event_id)\
