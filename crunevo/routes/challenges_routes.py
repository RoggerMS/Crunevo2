...
from crunevo.models.challenges import (
    GhostMentorChallenge, GhostMentorResponse, 
    MasterQuestion, MasterQuestionAttempt
)
...
def ghost_mentor():
    ...
    active_challenge = GhostMentorChallenge.query.filter(
        GhostMentorChallenge.is_active == True,
    ...
    recent_challenges = GhostMentorChallenge.query.filter(
        GhostMentorChallenge.end_time < datetime.utcnow()
    ).order_by(GhostMentorChallenge.created_at.desc()).limit(5).all()
    
    return render_template('challenges/ghost_mentor.html',
...
def submit_ghost_response():
    ...
    return jsonify({
        ...
        'is_correct': is_correct,
        'points_earned': points_earned,
        'message': '¡Respuesta enviada! Has ganado {} Crolars'.format(points_earned)
    })
...
def master_question():
    ...
    recent_questions = MasterQuestion.query.filter(
        MasterQuestion.active_date < today
    ).order_by(MasterQuestion.active_date.desc()).limit(7).all()
    
    return render_template('challenges/master_question.html',
                         question=question,
