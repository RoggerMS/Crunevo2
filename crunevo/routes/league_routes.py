...
def index():
    ...
    top_teams = (
        AcademicTeam.query.filter_by(is_active=True)
        .order_by(desc(AcademicTeam.points))
        .limit(10)
        .all()
    )
    ...
    current_league = LeagueMonth.query.filter(
        LeagueMonth.start_date <= datetime.now(), LeagueMonth.end_date >= datetime.now()
    ).first()
...
def create_team():
    ...
    if request.method == "POST":
        ...
        # Create team
        team = AcademicTeam(
            name=name, description=description, captain_id=current_user.id
        )
...
def team_detail(team_id):
    ...
    recent_actions = (
        TeamAction.query.filter_by(team_id=team_id)
        .order_by(desc(TeamAction.created_at))
        .limit(10)
        .all()
    )
...
def award_team_points(user_id, action_type, points):
    ...
    membership = TeamMember.query.filter_by(user_id=user_id, is_active=True).first()
