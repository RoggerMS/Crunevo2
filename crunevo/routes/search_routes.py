...
def search_posts(query, page=1, per_page=20):
    ...
    posts = base_query.filter(search_filter).order_by(
...
def search_users(query, page=1, per_page=20):
    ...
    for user in users.items:
        results.append({
            ...
            "about": user.about[:100] + "..." if user.about and len(user.about) > 100 else user.about,
...
def search_products(query, page=1, per_page=20):
    ...
    products = Product.query.filter(
        ...
        desc(Product.popularity_score) if hasattr(Product, 'popularity_score') else desc(Product.id)
    ).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def get_search_suggestions():
    ...
    return [
        "Matemáticas", "Física", "Química", "Programación", "Historia",
        "Literatura", "Biología", "Inglés", "Economía", "Psicología"
    ]
...
def search_suggestions():
    ...
    for note in tags_notes:
        if note.tags:
            tags = [tag.strip() for tag in note.tags.split(",")]
            for tag in tags:
                if query.lower() in tag.lower() and tag not in [s["text"] for s in suggestions]:
                    suggestions.append({
                        "text": tag,
                        "type": "tag",
                        "icon": "bi-tag",
...
def get_popular_subjects():
    ...
    return [
        ...
        "Literatura", "Inglés", "Programación", "Economía", "Psicología",
        "Filosofía", "Geografía", "Arte", "Música", "Educación Física"
    ]
