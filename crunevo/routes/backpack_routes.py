...
from crunevo.models.knowledge_backpack import KnowledgeBackpack, LearningEntry, BackpackAchievement
...
def index():
    ...
    saved_courses = current_user.saved_courses.count() if hasattr(current_user, 'saved_courses') else 0
    ...
    recent_entries = LearningEntry.query.filter_by(backpack_id=backpack.id).order_by(
        LearningEntry.created_at.desc()
    ).limit(5).all()
    
    # Get backpack achievements
    achievements = BackpackAchievement.query.filter_by(backpack_id=backpack.id).order_by(
        BackpackAchievement.earned_at.desc()
    ).all()
    
    return render_template('backpack/index.html',
                         backpack=backpack,
...
def journal():
    ...
    entries = LearningEntry.query.filter_by(backpack_id=backpack.id).order_by(
        LearningEntry.created_at.desc()
    ).all()
    
    return render_template('backpack/journal.html',
                         backpack=backpack,
                         entries=entries)
...
def export_pdf():
    ...
    # Title
    title = Paragraph(f"Bitácora de Aprendizaje - {current_user.username}", styles['Title'])
    story.append(title)
...
def get_stats():
    ...
    ).filter_by(backpack_id=backpack.id).group_by(
        extract('month', LearningEntry.created_at)
    ).all()
    
    # Get entry types distribution
    entry_types = db.session.query(
        LearningEntry.entry_type,
        db.func.count(LearningEntry.id).label('count')
    ).filter_by(backpack_id=backpack.id).group_by(
        LearningEntry.entry_type
    ).all()
    ...
    return jsonify({
        'monthly_entries': [{'month': m.month, 'count': m.count} for m in monthly_entries],
        'entry_types': [{'type': t.entry_type, 'count': t.count} for t in entry_types],
...
def check_journal_achievements(backpack_id):
    ...
    backpack = KnowledgeBackpack.query.get(backpack_id)
    ...
    achievements_to_check = [
        ...
        (50, 'knowledge_keeper', 'Guardián del Conocimiento', '50 entradas en tu bitácora')
    ]
