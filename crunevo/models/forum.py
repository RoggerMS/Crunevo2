...
question_tags = db.Table(
    "question_tags",
    db.Column(
        "question_id", db.Integer, db.ForeignKey("forum_question.id"), primary_key=True
    ),
...
answer_votes = db.Table(
    "answer_votes",
    db.Column("user_id", db.Integer, db.ForeignKey("user.id"), primary_key=True),
    db.Column(
        "answer_id", db.Integer, db.ForeignKey("forum_answer.id"), primary_key=True
    ),
...
class ForumQuestion(db.Model):
    ...
    difficulty_level = db.Column(
        db.String(20), default="intermedio"
...
class ForumAnswer(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    content = db.Column(db.Text, nullable=False)
    explanation_quality = db.Column(
        db.String(20), default="good"
    )  # poor, good, excellent
    has_step_by_step = db.Column(
        db.Boolean, default=False
...
class ForumReport(db.Model):
    ...
    )  # spam, inappropriate, off-topic, etc.
