...
class GhostMentorChallenge(db.Model):
    ...
    responses = db.relationship(
        "GhostMentorResponse", backref="challenge", lazy="dynamic"
    )
...
class GhostMentorResponse(db.Model):
    ...
    challenge_id = db.Column(
        db.Integer, db.ForeignKey("ghost_mentor_challenge.id"), nullable=False
    )
...
class MasterQuestion(db.Model):
    ...
    attempts = db.relationship(
        "MasterQuestionAttempt", backref="question", lazy="dynamic"
    )
...
class MasterQuestionAttempt(db.Model):
    ...
    question_id = db.Column(
        db.Integer, db.ForeignKey("master_question.id"), nullable=False
    )
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
