...
class Mentorship(db.Model):
    ...
    # Relationships
    mentor = relationship('User', foreign_keys=[mentor_id], backref='mentoring_sessions')
    student = relationship('User', foreign_keys=[student_id], backref='learning_sessions')
...
class Challenge(db.Model):
    ...
    target_action = Column(String(50), nullable=False)  # e.g., 'ask_questions', 'answer_questions'
    start_date = Column(DateTime, nullable=False)
    end_date = Column(DateTime, nullable=False)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
