...
class KnowledgeBackpack(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('knowledge_backpack', uselist=False))


class LearningEntry(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    backpack_id = db.Column(db.Integer, db.ForeignKey('knowledge_backpack.id'), nullable=False)
    title = db.Column(db.String(200), nullable=False)
    content = db.Column(db.Text, nullable=False)
    entry_type = db.Column(db.String(50), default='reflection')  # reflection, goal, achievement
