...
class MarketplaceProduct(db.Model):
    ...
    images = db.relationship('MarketplaceProductImage', backref='product', cascade='all, delete-orphan')
    sales = db.relationship('MarketplaceProductSale', backref='product', lazy='dynamic')
    
    @hybrid_property
    def main_image(self):
        """Get the main image for this product"""
        first_image = MarketplaceProductImage.query.filter_by(product_id=self.id).first()
        if first_image:
...
class MarketplaceProductSale(db.Model):
    ...
    status = db.Column(db.String(20), default='completed')  # pending, completed, cancelled, refunded
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class MarketplaceConversation(db.Model):
    ...
    messages = db.relationship('MarketplaceMessage', backref='conversation', lazy='dynamic', 
                              cascade='all, delete-orphan', order_by='MarketplaceMessage.created_at')
...
class MarketplaceMessage(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    conversation_id = db.Column(db.Integer, db.ForeignKey('marketplace_conversations.id'), nullable=False)
