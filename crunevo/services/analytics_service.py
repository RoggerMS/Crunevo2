...
class AnalyticsService:
    ...
    def get_dashboard_metrics(user_id: int) -> Dict[str, Any]:
        ...
        blocks = PersonalSpaceBlock.query.filter_by(user_id=user_id, status='active').all()
        ...
        completed_tasks = sum(1 for b in task_blocks if (b.metadata_json or {}).get('completed'))
    ...
    def get_productivity_metrics(user_id: int) -> Dict[str, Any]:
        ...
        task_blocks = [b for b in blocks if b.type == 'tarea']
        completed_tasks = [b for b in task_blocks if (b.metadata_json or {}).get('completed')]
        ...
        return {
            'task_completion': {
                ...
                'completion_rate': round((len(completed_tasks) / len(task_blocks)) * 100, 1) if task_blocks else 0,
            ...
            'objective_progress': {
                ...
                'average_progress': round(sum(obj['progress'] for obj in objective_progress) / len(objective_progress), 1) if objective_progress else 0
            },
    ...
    def _calculate_productivity_score(user_id: int) -> int:
        ...
        objective_blocks = [b for b in blocks if b.type == 'objetivo']
        if objective_blocks:
            avg_progress = sum((b.metadata_json or {}).get('progress', 0) for b in objective_blocks) / len(objective_blocks)
        ...
        organization_score = min((len(blocks) / 10) * 10, 10)  # Max 10 points for having blocks
    ...
    def _calculate_trends(user_id: int) -> Dict[str, float]:
        ...
        # Calculate trends
        blocks_trend = ((current_blocks - previous_blocks) / previous_blocks * 100) if previous_blocks > 0 else 0
    ...
    def _get_recent_completions(user_id: int) -> List[Dict[str, Any]]:
        ...
        recent_tasks = PersonalSpaceBlock.query.filter(
            and_(
                PersonalSpaceBlock.user_id == user_id,
                PersonalSpaceBlock.type == 'tarea',
                PersonalSpaceBlock.updated_at >= week_ago
            )
        ).order_by(PersonalSpaceBlock.updated_at.desc()).limit(10).all()
    ...
    def _is_goal_overdue(metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            return target_date < datetime.utcnow() and metadata.get('status') != 'cumplida'
