...
class BlockService:
    ...
    def create_block(user_id: int, block_data: Dict[str, Any]) -> PersonalSpaceBlock:
        ...
        max_order = db.session.query(func.max(PersonalSpaceBlock.order_index)).filter_by(
            user_id=user_id
        ).scalar() or 0
    ...
    @staticmethod
    def update_block(block_id: str, user_id: int, update_data: Dict[str, Any]) -> PersonalSpaceBlock:
    ...
    def get_user_blocks(user_id: int, status: str = 'active', 
                       block_type: Optional[str] = None) -> List[PersonalSpaceBlock]:
    ...
    def search_blocks(user_id: int, search_term: str) -> List[PersonalSpaceBlock]:
        ...
        return PersonalSpaceBlock.query.filter(
            and_(
        ...
        ).order_by(PersonalSpaceBlock.updated_at.desc()).all()
    ...
    def get_block_analytics(user_id: int) -> Dict[str, Any]:
        ...
        if objective_blocks:
            total_progress = sum((b.metadata_json or {}).get('progress', 0) for b in objective_blocks)
        ...
        return {
            ...
            'task_completion': {
                ...
                'percentage': int((completed_tasks / len(task_blocks)) * 100) if task_blocks else 0
            },
            'objective_progress': {
            ...
            'recent_activity': {
                'blocks_updated_this_week': len(recent_blocks)
            }
        }