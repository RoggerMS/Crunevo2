...
class RecommendationService:
    ...
    def get_personalized_recommendations(user_id: int, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            recent_recommendations = RecommendationService._get_activity_based_recommendations(user_id, limit // 3)
            recommendations.extend(recent_recommendations)
            
            # 2. Recomendaciones basadas en intereses (tags)
            interest_recommendations = RecommendationService._get_interest_based_recommendations(user_id, limit // 3)
            recommendations.extend(interest_recommendations)
            
            # 3. Recomendaciones colaborativas (usuarios similares)
            collaborative_recommendations = RecommendationService._get_collaborative_recommendations(user_id, limit // 3)
            recommendations.extend(collaborative_recommendations)
            
            # 4. Recomendaciones de contenido popular
            if len(recommendations) < limit:
                popular_recommendations = RecommendationService._get_popular_content_recommendations(user_id, limit - len(recommendations))
    ...
    def get_similar_questions(question_id: int, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            similar_questions = db.session.query(ForumQuestion).join(
                ForumQuestion.tags
            ).filter(
                and_(
                    ForumQuestion.id != question_id,
                    ForumTag.name.in_(current_tags)
                )
            ).group_by(ForumQuestion.id).order_by(
            ...
            for similar_q in similar_questions:
                ...
                common_tags = len(set(current_tags) & set([tag.name for tag in similar_q.tags]))
                category_match = 1 if similar_q.category == question.category else 0
                difficulty_match = 1 if similar_q.difficulty_level == question.difficulty_level else 0
                
                score = (common_tags * 0.5) + (category_match * 0.3) + (difficulty_match * 0.2)
    ...
    def get_trending_topics(days: int = 7, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            trending_tags = db.session.query(
                ...
                question_tags, ForumTag.id == question_tags.c.tag_id
            ).join(
                ForumQuestion, question_tags.c.question_id == ForumQuestion.id
            ).filter(
                ForumQuestion.created_at >= start_date
            ).group_by(ForumTag.name).order_by(
                desc(func.count(ForumTag.name))
            ).limit(limit).all()
            ...
            trending_categories = db.session.query(
                ...
                ForumQuestion.created_at >= start_date
            ).group_by(ForumQuestion.category).order_by(
                desc(func.count(ForumQuestion.id))
            ).limit(limit).all()
    ...
    @staticmethod
    def get_study_recommendations_for_user(user_id: int, subject: str = None, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for area in weak_areas:
                ...
                unanswered_questions = ForumQuestion.query.filter(
                    and_(
            ...
            solved_questions = ForumQuestion.query.filter(
                and_(
                    ForumQuestion.is_solved == True,
                    ForumQuestion.category.ilike(f'%{subject}%') if subject else True,
                    ForumQuestion.author_id != user_id
                )
            ).order_by(
                desc(ForumQuestion.views),
                desc(ForumQuestion.answer_count)
            ).limit(3).all()
    ...
    @staticmethod
    def get_content_recommendations_by_performance(user_id: int, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            performance_by_category = RecommendationService._analyze_user_performance(user_id)
            ...
            for category, performance in performance_by_category.items():
                if performance['avg_quality'] < 60:  # Área de mejora
                    ...
                    educational_questions = ForumQuestion.query.filter(
                        and_(
                            ForumQuestion.category == category,
                            ForumQuestion.is_solved == True,
                            ForumQuestion.difficulty_level == 'basico'
                        )
                    ).order_by(desc(ForumQuestion.views)).limit(2).all()
                    ...
                    challenging_questions = ForumQuestion.query.filter(
                        and_(
                            ForumQuestion.category == category,
                            ForumQuestion.difficulty_level == 'avanzado',
                            ForumQuestion.answer_count == 0
                        )
                    ).order_by(desc(ForumQuestion.bounty_points)).limit(1).all()
    ...
    def _get_activity_based_recommendations(user_id: int, limit: int) -> List[Dict]:
        ...
        try:
            ...
            recent_activity = db.session.query(
                ...
                or_(
                    ...
                    ForumQuestion.id.in_(
                        db.session.query(ForumAnswer.question_id).filter_by(author_id=user_id)
                    )
                )
            ).filter(
            ...
            for category, activity_count in recent_activity:
                ...
                ).order_by(desc(ForumQuestion.created_at)).limit(2).all()
    ...
    def _get_interest_based_recommendations(user_id: int, limit: int) -> List[Dict]:
        ...
        try:
            ...
            for tag_name, usage_count in user_tags:
                ...
                questions = db.session.query(ForumQuestion).join(
                    ForumQuestion.tags
                ).filter(
                    and_(
                        ForumTag.name == tag_name,
                        ForumQuestion.author_id != user_id,
                        ForumQuestion.answer_count < 3  # Preguntas que necesitan más respuestas
                    )
                ).order_by(desc(ForumQuestion.created_at)).limit(2).all()
    ...
    def _get_collaborative_recommendations(user_id: int, limit: int) -> List[Dict]:
        ...
        try:
            ...
            for similar_user_id, similarity_score in similar_users:
                ...
                good_answers = db.session.query(ForumAnswer).filter(
                    and_(
                        ForumAnswer.author_id == similar_user_id,
                        ForumAnswer.is_best_answer == True
                    )
                ).limit(2).all()
    ...
    def _get_popular_content_recommendations(user_id: int, limit: int) -> List[Dict]:
        ...
        try:
            ...
            popular_questions = ForumQuestion.query.filter(
                ForumQuestion.author_id != user_id
            ).order_by(
                desc(ForumQuestion.views),
                desc(ForumQuestion.answer_count)
    ...
    def _find_similar_users(user_id: int, limit: int) -> List[Tuple[int, float]]:
        ...
        try:
            ...
            ).group_by(ForumQuestion.category).all()
            
            user_category_counts = {category: count for category, count in user_categories}
            ...
            for category in user_category_counts.keys():
                other_users = db.session.query(
                    ...
                    ForumQuestion, User.id == ForumQuestion.author_id
                ).filter(
                    and_(
                        ForumQuestion.category == category,
                        User.id != user_id
                ...
                for other_user_id, activity_count in other_users:
                    ...
                    similarity = min(activity_count, user_category_counts[category]) / max(activity_count, user_category_counts[category])
    ...
    def _identify_weak_areas(user_id: int, subject: str = None) -> List[str]:
        ...
        try:
            ...
            performance_query = db.session.query(
                ...
                ForumAnswer, ForumQuestion.id == ForumAnswer.question_id
            ).filter(
            ...
            performance_data = performance_query.group_by(
                ForumQuestion.category
            ).all()
        ...
        try:
            ...
            for category, avg_quality, answer_count, best_answers in performance_data:
                performance[category] = {
                    ...
                    'success_rate': (best_answers / answer_count * 100) if answer_count > 0 else 0
                }
