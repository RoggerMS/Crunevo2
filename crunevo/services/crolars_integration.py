...
from crunevo.models.social import Challenge, UserChallenge
...
class CrolarsIntegrationService:
    ...
    PREMIUM_COSTS = {"boost_question": 100, "highlight_answer": 75, "custom_title": 200}
    ...
    def get_user_crolars_stats(user_id: int) -> Dict:
        ...
        helpful_votes = (
            db.session.query(func.sum(ForumAnswer.helpful_count))
        ...
        estimated_forum_crolars = (
            ...
            + answers_count
            * CrolarsIntegrationService.ACTIVITY_REWARDS["answer_question"]
            + best_answers_count
            * CrolarsIntegrationService.ACTIVITY_REWARDS["best_answer"]
    ...
    def get_crolars_leaderboard(limit: int = 10) -> List[Dict]:
        ...
        for i, user in enumerate(users, 1):
            ...
            estimated_earned = (
                questions_count
                * CrolarsIntegrationService.ACTIVITY_REWARDS["ask_question"]
                + answers_count
                * CrolarsIntegrationService.ACTIVITY_REWARDS["answer_question"]
                + best_answers_count
                * CrolarsIntegrationService.ACTIVITY_REWARDS["best_answer"]
    ...
    @staticmethod
    def award_crolars(
        user_id: int, activity: str, amount: Optional[int] = None
    ) -> bool:
    ...
    def get_active_challenges(user_id: int) -> List[Dict]:
        ...
        active_challenges = Challenge.query.filter(
            Challenge.is_active, Challenge.start_date <= now, Challenge.end_date >= now
        ).all()
        ...
        for challenge in active_challenges:
            ...
            if not user_challenge:
                ...
                user_challenge = UserChallenge(
                    user_id=user_id, challenge_id=challenge.id, progress=0
                )
            ...
            user_challenges.append(
                {
                    ...
                    "progress_percentage": min(
                        100, (progress / challenge.target_value) * 100
                    ),
    ...
    def _calculate_challenge_progress(
        user_id: int, challenge: Challenge, user_challenge: UserChallenge
    ) -> int:
        ...
        if challenge.target_action == "ask_questions":
            ...
            return ForumAnswer.query.filter(
                ForumAnswer.author_id == user_id, ForumAnswer.created_at >= start_date
            ).count()
