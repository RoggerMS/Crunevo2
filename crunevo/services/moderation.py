...
class ModerationService:
    ...
    SPAM_KEYWORDS = [
        'gratis', 'dinero', 'ganar', 'click', 'enlace', 'link',
        'promocion', 'oferta', 'descuento', 'premio'
    ]
    ...
    def analyze_content_quality(content: str, title: str = "") -> Dict[str, any]:
        ...
        # 3. Verificar spam
        spam_count = sum(1 for keyword in ModerationService.SPAM_KEYWORDS 
    ...
    def check_user_behavior(user_id: int) -> Dict[str, any]:
        ...
        return {
            ...
            'risk_level': 'high' if len(suspicious_reasons) >= 3 else 'medium' if len(suspicious_reasons) >= 2 else 'low'
        }
    ...
    def auto_moderate_question(question: ForumQuestion) -> Dict[str, any]:
        ...
        return {
            ...
            'confidence': ModerationService._calculate_confidence(content_analysis, user_analysis)
        }
    ...
    def get_moderation_stats() -> Dict[str, any]:
        ...
        high_quality = db.session.query(func.count(ForumQuestion.id)).filter(
            ForumQuestion.quality_score >= 80
        ).scalar() or 0
        
        medium_quality = db.session.query(func.count(ForumQuestion.id)).filter(
            ForumQuestion.quality_score.between(60, 79)
        ).scalar() or 0
        
        low_quality = db.session.query(func.count(ForumQuestion.id)).filter(
            ForumQuestion.quality_score < 60
        ).scalar() or 0
    ...
    def suggest_improvements(content: str, issues: List[str]) -> List[str]:
        ...
        if "Contenido muy corto" in issues:
            suggestions.append("Proporciona más detalles sobre tu pregunta. Incluye contexto, lo que has intentado y dónde tienes dificultades.")
        
        if "Contiene patrones de baja calidad" in issues:
            suggestions.append("Evita usar palabras como 'ayuda', 'urgente' o 'rápido'. En su lugar, sé específico sobre lo que necesitas.")
        
        if "Demasiadas mayúsculas" in issues:
            suggestions.append("Evita escribir en mayúsculas. Usa mayúsculas solo al inicio de oraciones y para nombres propios.")
        
        if "Uso excesivo de signos de puntuación" in issues:
            suggestions.append("Usa signos de puntuación con moderación. Un solo signo de interrogación o exclamación es suficiente.")
        
        if "Falta de estructura en el texto" in issues:
            suggestions.append("Organiza tu texto en párrafos. Usa puntos y comas para hacer tu pregunta más clara.")
        
        if not suggestions:
            suggestions.append("Tu contenido se ve bien. Considera agregar más detalles si es necesario.")
