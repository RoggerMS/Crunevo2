...
class LearningToolsService:
    ...
    def get_user_learning_stats(user_id: int) -> Dict:
        ...
        try:
            ...
            solved_questions = db.session.query(ForumQuestion).join(
            ...
            # Tasa de éxito
            success_rate = (solved_questions / answers_given * 100) if answers_given > 0 else 0
            ...
            next_level_xp = LearningToolsService._calculate_xp_for_level(current_level + 1)
            current_level_xp = LearningToolsService._calculate_xp_for_level(current_level)
            progress_to_next = ((current_xp - current_level_xp) / (next_level_xp - current_level_xp) * 100) if next_level_xp > current_level_xp else 100
    ...
    def get_study_recommendations(user_id: int, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for category in weak_categories[:3]:
                ...
                ).order_by(ForumQuestion.created_at.desc()).limit(2).all()
                ...
                for question in unanswered_questions:
                    recommendations.append({
                        ...
                        'points': 10 + (5 if question.difficulty_level == 'hard' else 0)
                    })
        ...
        try:
            ...
            learning_steps = [
                ...
                },
                {
                    ...
                    'description': f'Aplica tus conocimientos en problemas reales',
                    'difficulty': 'medium',
    ...
    @staticmethod
    def track_study_session(user_id: int, activity_type: str, duration_minutes: int, topics: List[str]) -> bool:
    ...
    def get_performance_analytics(user_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            ).group_by(func.date(ForumQuestion.created_at)).all()
            ...
            ).group_by(func.date(ForumAnswer.created_at)).all()
            
            # Calidad promedio
            avg_question_quality = db.session.query(
                func.avg(ForumQuestion.quality_score)
            ).filter(
                and_(
                    ForumQuestion.author_id == user_id,
                    ForumQuestion.created_at >= start_date
            ...
            avg_answer_quality = db.session.query(
                func.avg(ForumAnswer.quality_score)
            ).filter(
                and_(
                    ForumAnswer.author_id == user_id,
                    ForumAnswer.created_at >= start_date
            ...
            early_activity = ForumQuestion.query.filter(
                and_(
                    ForumQuestion.author_id == user_id,
                    ForumQuestion.created_at >= start_date,
                    ForumQuestion.created_at < first_half
                )
            ).count() + ForumAnswer.query.filter(
                and_(
                    ForumAnswer.author_id == user_id,
                    ForumAnswer.created_at >= start_date,
            ...
            recent_activity = ForumQuestion.query.filter(
                and_(
                    ForumQuestion.author_id == user_id,
                    ForumQuestion.created_at >= first_half
                )
            ...
            activity_trend = 'increasing' if recent_activity > early_activity else 'decreasing' if recent_activity < early_activity else 'stable'
            
            return {
                'period_days': days,
                'daily_questions': [{'date': str(date), 'count': count} for date, count in daily_activity],
                'daily_answers': [{'date': str(date), 'count': count} for date, count in daily_answers],
    ...
    def _assess_subject_level(user_id: int, subject: str) -> int:
        ...
        try:
            ...
            answers_in_subject = db.session.query(ForumAnswer).join(
                ForumQuestion, ForumAnswer.question_id == ForumQuestion.id
            ).filter(
                and_(
                    ForumAnswer.author_id == user_id,
                    ForumQuestion.category.ilike(f'%{subject}%')
