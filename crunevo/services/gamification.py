...
class GamificationService:
    ...
    def award_experience(user, action, amount=None):
        ...
        if amount > 0:
            ...
            if level_up:
                flash(f'¡Felicidades! Has subido al nivel {user.forum_level}: {user.get_forum_rank()}!', 'success')
    ...
    def process_question_action(user, is_first=False, question=None):
        ...
        if is_first:
            ...
            if question:
                crolars_amount = CrolarsIntegrationService.calculate_question_reward(question)
                CrolarsIntegrationService.award_crolars_for_action(user, 'ask_question', crolars_amount)
            else:
    ...
    def process_answer_action(user, is_first=False, answer=None):
        ...
        if is_first:
            ...
            if answer:
                crolars_amount = CrolarsIntegrationService.calculate_answer_reward(answer)
                CrolarsIntegrationService.award_crolars_for_action(user, 'answer_question', crolars_amount)
            else:
                CrolarsIntegrationService.award_crolars_for_action(user, 'answer_question')
    ...
    def process_best_answer(user, answer=None):
        ...
        if answer:
            crolars_amount = CrolarsIntegrationService.calculate_answer_reward(answer, is_best=True)
            CrolarsIntegrationService.award_crolars_for_action(user, 'best_answer', crolars_amount)
        else:
