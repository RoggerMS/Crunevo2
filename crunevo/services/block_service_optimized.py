...
from crunevo.models import PersonalSpaceBlock, User
from crunevo.utils.helpers import generate_uuid
...
class OptimizedBlockService:
    ...
    def create_block(user_id: int, block_data: Dict[str, Any]) -> PersonalSpaceBlock:
        ...
        try:
            ...
            max_order_result = db.session.execute(
                text("SELECT COALESCE(MAX(order_index), 0) FROM personal_space_blocks WHERE user_id = :user_id"),
                {'user_id': user_id}
            ).scalar()
            ...
            block = PersonalSpaceBlock(
                ...
                metadata_json=OptimizedBlockService._get_enhanced_metadata(cleaned_data),
            ...
            OptimizedBlockService._log_block_action('create', user_id, block.id, block.type)
    ...
    def _validate_block_data(block_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(content) > config['max_content_length']:
            errors.append(f"Content cannot exceed {config['max_content_length']} characters")
        else:
        ...
        if isinstance(metadata, dict):
            cleaned_data['metadata'] = OptimizedBlockService._validate_metadata(metadata, block_type)
        else:
    ...
    def _validate_metadata(metadata: Dict[str, Any], block_type: str) -> Dict[str, Any]:
        ...
        if block_type == 'tarea':
            ...
            if 'priority' in metadata and metadata['priority'] in ['low', 'medium', 'high', 'urgent']:
            ...
            if 'status' in metadata and metadata['status'] in ['no_iniciada', 'en_progreso', 'cumplida']:
    ...
    def _get_enhanced_metadata(cleaned_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add computed fields
        if block_type == 'tarea':
            metadata['estimated_duration'] = metadata.get('estimated_duration', 60)  # minutes
        elif block_type == 'objetivo':
    ...
    @staticmethod
    @with_cache(lambda user_id, **kwargs: f"user_{user_id}_blocks_{kwargs.get('status', 'active')}_{kwargs.get('block_type', 'all')}")
    def get_user_blocks(user_id: int, status: str = 'active', 
                       block_type: Optional[str] = None, 
                       include_metadata: bool = True) -> List[Dict[str, Any]]:
        """Get user blocks with optimized querying and caching."""
        try:
            # Build optimized query
            query = db.session.query(PersonalSpaceBlock).filter_by(user_id=user_id, status=status)
    ...
    @staticmethod
    def update_block(block_id: str, user_id: int, update_data: Dict[str, Any]) -> Optional[PersonalSpaceBlock]:
        """Update a block with optimized validation and caching."""
        try:
            # Use optimized query with explicit user filter for security
            block = db.session.query(PersonalSpaceBlock).filter(
                and_(
            ...
            if update_fields:
                ...
                db.session.query(PersonalSpaceBlock).filter_by(id=block_id).update(update_fields)
                db.session.commit()
            ...
            OptimizedBlockService._log_block_action('update', user_id, block.id, block.type)
            
            return block
            
        except Exception as e:
            db.session.rollback()
            logging.error(f"Error updating block {block_id} for user {user_id}: {str(e)}")
            raise
    
    @staticmethod
    def bulk_update_blocks(user_id: int, updates: List[Dict[str, Any]]) -> Tuple[int, List[str]]:
        ...
        try:
            for update in updates:
                ...
                try:
                    result = OptimizedBlockService.update_block(block_id, user_id, update)
                    if result:
                        updated_count += 1
                    else:
    ...
    @staticmethod
    def _log_block_action(action: str, user_id: int, block_id: str, block_type: str) -> None:
        """Log block actions for analytics (async in production)."""
        try:
            # In production, this would be sent to an analytics service
            logging.info(f"Block action: {action} - User: {user_id} - Block: {block_id} - Type: {block_type}")
        except Exception:
    ...
    def cleanup_deleted_blocks(days_old: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = db.session.query(PersonalSpaceBlock).filter(
                and_(
                    PersonalSpaceBlock.status == 'deleted',
            ...
            logging.info(f"Cleaned up {deleted_count} deleted blocks older than {days_old} days")
