...
class QueryProfiler:
    ...
    def record_query(self, query_hash: str, execution_time: float, 
                    rows_returned: int, cache_hit: bool, query_type: str):
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        recent_stats = [s for s in self.stats if s.timestamp > timezone.now() - timedelta(hours=1)]
        
        avg_execution_time = sum(s.execution_time for s in recent_stats) / len(recent_stats)
        cache_hit_rate = (self.cache_stats['hits'] / self.cache_stats['total_queries']) * 100
...
class AdvancedCache:
    ...
    def set(self, key: str, value: Any, timeout: Optional[int] = None, tags: Optional[List[str]] = None):
...
def cache_query(timeout: int = 300, tags: Optional[List[str]] = None, 
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            # Generate cache key
            cache_key = advanced_cache.get_cache_key(key_prefix, func.__name__, *args, **kwargs)
            ...
            if cached_result is not None:
                ...
                query_profiler.record_query(
                    ...
                    execution_time=execution_time,
                    rows_returned=len(cached_result) if isinstance(cached_result, (list, tuple)) else 1,
                    cache_hit=True,
                    query_type='cached'
                )
...
class DatabaseOptimizer:
    ...
    def get_user_blocks_optimized(self, user_id: int, limit: int = 50, offset: int = 0) -> List[Dict]:
    ...
    @cache_query(timeout=600, tags=['blocks', 'analytics'], key_prefix='block_analytics')
    ...
    def search_blocks_optimized(self, user_id: int, query: str, block_types: Optional[List[str]] = None,
    ...
    def bulk_update_blocks(self, user_id: int, updates: List[Dict[str, Any]]) -> bool:
        ...
        try:
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    for update in updates:
                        ...
                        for field, value in update.items():
                            if field != 'id':
                                if field == 'metadata':
                                    update_cases.append(f"WHEN id = {block_id} THEN '{json.dumps(value)}'::jsonb")
                                else:
                ...
                self.cache.invalidate_by_tags(['blocks', 'user_blocks', f'user_{user_id}'])
    ...
    def bulk_reorder_blocks(self, user_id: int, block_positions: List[Tuple[int, int]]) -> bool:
        """Efficiently reorder multiple blocks"""
        try:
            with transaction.atomic():
                with connection.cursor() as cursor:
                    # Use VALUES clause for efficient bulk update
                    values_clause = ','.join([f"({block_id}, {position})" 
    ...
    def cleanup_old_stats(self, days: int = 7):
        ...
        cutoff_date = timezone.now() - timedelta(days=days)
        self.profiler.stats = [
            stat for stat in self.profiler.stats 
