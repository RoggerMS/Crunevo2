...
class Analytics:
    ...
    def get_engagement_metrics(days=7):
        ...
        metrics = {
            ...
            "total_likes": db.session.query(func.sum(Post.likes))
            .filter(Post.created_at >= since)
            .scalar()
            or 0,
            "active_users": db.session.query(func.count(func.distinct(Post.author_id)))
            .filter(Post.created_at >= since)
            .scalar()
            or 0,
        }
    ...
    def get_revenue_metrics(days=30):
        ...
        revenue = (
            db.session.query(func.sum(Purchase.total_credits), func.count(Purchase.id))
