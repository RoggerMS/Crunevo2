...
from crunevo.models.marketplace import Seller, MarketplaceProduct, MarketplaceProductImage
...
def get_unread_messages_count(user_id):
    ...
    for conversation in conversations:
        unread_count += MarketplaceMessage.query.filter_by(
            ...
            is_read=False
        ).filter(MarketplaceMessage.sender_id != user_id).count()
...
def get_recent_sales(seller_id, limit=5):
    ...
    sales = MarketplaceProductSale.query.filter(
        MarketplaceProductSale.product_id.in_(product_ids)
    ).order_by(MarketplaceProductSale.created_at.desc()).limit(limit).all()
...
def get_recent_messages(user_id, limit=5):
    ...
    ).order_by(MarketplaceMessage.created_at.desc()).limit(limit).all()
