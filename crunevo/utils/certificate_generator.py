...
def generate_certificate_pdf(user, certificate_type):
    ...
    profile_url = f"https://crunevo.com/perfil/{user.username}"  # Adjust domain as needed
    ...
    footer_data = [
        ['Firma Digital CRUNEVO', ''],
        ['Verificar en:', qr_buffer.getvalue()]
    ]
    
    content.append(Paragraph("___________________", body_style))
    content.append(Paragraph("Equipo CRUNEVO", body_style))
    content.append(Spacer(1, 20))
    content.append(Paragraph(f"QR: Perfil de {user.username}", 
