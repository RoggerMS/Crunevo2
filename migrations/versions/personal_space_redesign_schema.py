...
def upgrade():
    ...
    op.create_table('personal_space_blocks',
        ...
        sa.CheckConstraint("type IN ('tarea', 'nota', 'kanban', 'objetivo', 'bitacora', 'enlace', 'frase', 'recordatorio', 'meta', 'lista', 'nota_enriquecida', 'bloque_personalizado')", name='check_block_type'),
        sa.CheckConstraint("status IN ('active', 'completed', 'archived', 'deleted')", name='check_block_status'),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create indexes for optimization
    op.create_index('idx_personal_space_blocks_user_id', 'personal_space_blocks', ['user_id'])
    op.create_index('idx_personal_space_blocks_type', 'personal_space_blocks', ['type'])
    op.create_index('idx_personal_space_blocks_status', 'personal_space_blocks', ['status'])
    op.create_index('idx_personal_space_blocks_order', 'personal_space_blocks', ['user_id', 'order_index'])
    op.create_index('idx_personal_space_blocks_updated_at', 'personal_space_blocks', ['updated_at'], postgresql_using='btree', postgresql_ops={'updated_at': 'DESC'})
    
    # Create GIN index for JSONB metadata
    op.create_index('idx_personal_space_blocks_metadata', 'personal_space_blocks', ['metadata'], postgresql_using='gin')
    
    # Create personal_space_templates table
    op.create_table('personal_space_templates',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
    ...
    op.create_table('personal_space_block_attachments',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=True, server_default=sa.text('NOW()')),
        sa.ForeignKeyConstraint(['block_id'], ['personal_space_blocks.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create personal_space_analytics_events table
    op.create_table('personal_space_analytics_events',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('event_type', sa.String(length=100), nullable=False),
        sa.Column('event_data', postgresql.JSONB(astext_type=sa.Text()), nullable=True, server_default=sa.text("'{}'") ),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=True, server_default=sa.text('NOW()')),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index('idx_personal_space_analytics_user_id', 'personal_space_analytics_events', ['user_id'])
    op.create_index('idx_personal_space_analytics_event_type', 'personal_space_analytics_events', ['event_type'])
    op.create_index('idx_personal_space_analytics_created_at', 'personal_space_analytics_events', ['created_at'])
