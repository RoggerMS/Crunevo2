...
down_revision = "new_sections_models"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not has_column("club", "banner_url", conn):
        op.add_column("club", sa.Column("banner_url", sa.String(length=255), nullable=True))
    
    # Add facebook_url column if it doesn't exist
    if not has_column("club", "facebook_url", conn):
        op.add_column("club", sa.Column("facebook_url", sa.String(length=255), nullable=True))
    
    # Add whatsapp_url column if it doesn't exist
    if not has_column("club", "whatsapp_url", conn):
        op.add_column("club", sa.Column("whatsapp_url", sa.String(length=255), nullable=True))
