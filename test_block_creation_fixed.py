    ...
    try:
        ...
        if not csrf_token:
            csrf_match = re.search(
                r'csrf_token["\']\s*:\s*["\']([^"\'\']+)["\']', login_response.text
            )
            if csrf_match:
        ...
        if not new_csrf_token:
            csrf_match = re.search(
                r'csrf_token["\']\s*:\s*["\']([^"\'\']+)["\']', ps_response.text
            )
            if csrf_match:
        ...
        block_data = {
            ...
            "metadata": {"color": "blue", "icon": "bi-journal-text"},
        }
        ...
        create_response = session.post(
            f"{base_url}/api/personal-space/blocks", json=block_data, headers=headers
        )
        ...
        if create_response.status_code == 200:
            try:
                ...
                if response_data.get("success"):
                    ...
                    print(
                        f"❌ Error en la respuesta: {response_data.get('message', 'Error desconocido')}"
                    )
                    return False
