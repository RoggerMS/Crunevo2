...
def create_sample_challenges():
    ...
    with app.app_context():
        ...
        if existing_challenges > 0:
            print(
                f"Found {existing_challenges} existing challenges. Skipping creation."
            )
            return
        ...
        try:
            ...
            print("\nCreated challenges:")
            daily_challenges = Challenge.query.filter_by(
                challenge_type=ChallengeType.DAILY
            ).all()
            weekly_challenges = Challenge.query.filter_by(
                challenge_type=ChallengeType.WEEKLY
            ).all()
